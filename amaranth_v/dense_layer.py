...
class QDenseLayer(wiring.Component):
    ...
    Weights are stored in NNQ; the running accumulator and biases are held in
    the double-width NNQ_DW (qkeras uses a double-width bias quantiser, so the
    raw-integer bias aligns exactly with the a*b product scale).
    """
    ...
    def __init__(
        ...
        print(
            ...
            f" ( relu_upper_bound={relu_upper_bound} ) out_shape={out_shape!r}"
        )
        ...
        # biases (double width). when absent, seed with zeros.
        if np_bias is not None:
            bias_init = [parse_nnq(b, shape=NNQ_DW) for b in np_bias]
        else:
            bias_init = [parse_nnq(0.0, shape=NNQ_DW) for _ in range(self.OUT_D)]
        ...
        self.accumulator = Signal(NNQ_DW, name="qdense_running_accum", init=0)

        self.input = Array(
            Signal(NNQ, name=f"qdense_in_{i}", init=0) for i in range(self.IN_D)
        )
        ...
        # clip to representable out_shape bounds (expressed in NNQ_DW shape)
        # before narrowing NNQ_DW -> out_shape. note: can't use fixed Value utils
        # (clamp) directly since we're matching FXPmath/qkeras which does things
        # slightly differently.
        self.lower_bound = fixed.Const(
            out_shape.min().as_float(), shape=NNQ_DW
        ).as_value()
        self.upper_bound = fixed.Const(
            out_shape.max().as_float(), shape=NNQ_DW
        ).as_value()
    ...
    def elaborate(self, platform):
        ...
        mul_a = Signal(signed(NNQ.width), name="qdense_mul_a")
        ...
        frac_drop = NNQ_DW.f_bits - self.out_shape.f_bits
