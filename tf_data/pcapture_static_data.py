...
class ParametricCaptureStaticData(object):
    ...
    def __init__(
        ...
        self.inr_model_data_z = zarr.open(
            zarr_base_path_for(capture_run) / "inr_model_data.z", mode="r"
        )
        self.n_chunks = self.inr_model_data_z.nchunks
        self.seq_len = self.inr_model_data_z.blocks[0].shape[0]
    ...
    def model_data_block_to_xs_ys(
        ...
        """
        Args:
            data: chunk from zarr inr_model_data.z
    ...
    def slice_window(
        ...
        block = self.inr_model_data_z.blocks[idx]
        data = block[seq_from:seq_to]
    ...
    def tf_inference_dataset(
        ...
        def sample_generator():
            for c in range(self.n_chunks):
                data = self.inr_model_data_z.blocks[c]
