...
export RUN=189_tune_2_siren10

export NFF=512
export MLP_DIMS="32 32 32"
export FILM_LAYERS=1
export MLP_ACT_FN=siren

# run initial keras_v model
uv run -m keras_v.train \
 --run $RUN/kv \
 --dataset-type embed2d --harsh \
 --num-fourier-features $NFF --rff-basis gaussian --rff-scale-min 0.1 --rff-scale-max 5.0 \
 --mlp-dims $MLP_DIMS --film-layers $FILM_LAYERS \
 --alpha-mse 0.0 --alpha-huber 1.0 \
 --mlp-activation siren --siren-omega 30 \
...
uv run -m qkeras_v.train \
 --run $RUN/qkv \
 --dataset-type embed2d --harsh \
 --init-from-run $RUN/kv --num-fourier-features $NFF --rff-lut-size 4096 \
 --io-fp-int 1 --io-fp-frac 14 \
 --mlp-fp-int 3 --mlp-fp-frac 13 --relu-upper-bound 8 \
 --mlp-dims $MLP_DIMS --film-layers $FILM_LAYERS \
 --alpha-mse 0.0 --alpha-huber 1.0 \
 --mlp-activation siren --siren-omega 10 \
 --beta-stft-warmup 5 --beta-stft-ramp 5 --beta-stft 0.001 \
 --base-stft-fft-size 2048 --base-stft-win-length 256 \
 --gamma-slope 0.1 --delta-dc 1e-4 \
 --epochs 20 --learning-rate 1e-4 --cosine-schedule \
